import React, { Component } from 'react';
import './App.css';
import AddToCart from './Components/AddToCart'


class App extends Component {
  state = {
    inventory: [
      {Name: 'Cereal',
      Img: 'https://images-na.ssl-images-amazon.com/images/G/01/aplusautomation/vendorimages/c3d80660-ba9e-4f42-b989-a423ba979fc2.jpg._CB289958391_.jpg',
      Price: '4.00'},
      {Name: 'Bananas',
      Img: 'https://img.purch.com/rc/300x200/aHR0cDovL3d3dy5saXZlc2NpZW5jZS5jb20vaW1hZ2VzL2kvMDAwLzA2NS8xNDkvb3JpZ2luYWwvYmFuYW5hcy5qcGc=',
      Price: '2.00'},
      {Name:'Steak',
      Img: 'https://cdn.shopify.com/s/files/1/1579/5769/products/ribeye_prime_1_1024x1024.jpg?v=1479226988',
      Price: '8.00'},
      {Name:'Orange Juice',
      Img: 'https://instamartkauai.com/wp-content/uploads/2018/04/large_add3b7ed-a890-485b-827a-294c0a3a740e-600x600.jpg',
      Price: '3.50'},
      {Name: 'Bread',
      Img: 'https://e22d0640933e3c7f8c86-34aee0c49088be50e3ac6555f6c963fb.ssl.cf2.rackcdn.com/0072250037060_CL_default_default_large.jpeg',
      Price: '1.00'},
      {Name: 'Milk',
      Img: 'data:image/jpeg;base64,',
      Price: '2.50'},
      {Name: 'Tomatoes',
      Img: 'https://cdn.shopify.com/s/files/1/2918/4630/products/Image_3_104b31f8-80a1-44f7-b624-56a429a073f9.jpg?v=1527364299',
      Price: '2.70'},
      {Name: 'Cheese',
      Img: 'https://static1.squarespace.com/static/5a1592ff0abd04e470d48744/t/5a27bbeeec212d905273a14c/1512996237223/Kaas.jpeg?format=1500w',
      Price: '4.00'},
      {Name: 'Ice Cream',
      Img: 'https://i5.walmartimages.com/asr/05613edd-98e9-453d-9e94-cb1ebdd5a841_1.f7e72f0841619ffcb1e30e5482a23e00.jpeg?odnHeight=450&odnWidth=450&odnBg=FFFFFF',
      Price: '2.99'},
      {Name: 'Chicken',
      Img: 'http://robbieflexibles.com/getmedia/bb525202-dcf5-40ae-b369-03b36ed92dc5/rotisserie-website.aspx',
      Price: '3.50'},
      {Name: 'Pizza',
      Img: 'https://www.dollargeneral.com/media/catalog/product/cache/image/700x700/e9c3970ab036de70892d86c6d221abfe/0/0/00071921825881_a1l1_900_900.jpg',
      Price: '5.00'},
      {Name: 'Lettuce',
      Img: 'https://images-na.ssl-images-amazon.com/images/I/41CGtIyWgML._SX355_.jpg',
      Price: '1.29'}],
    cartItems: []
  }
  

  checkoutAlert = () => {
    alert(`Are you sure you're ready to check out?`)
  }

  componentDidMount() {
    const cartJSON = localStorage.getItem('cartItems6');
    const cart = JSON.parse(cartJSON);
    this.setState({
        inventory: [...this.state.inventory],
        cartItems: cart || [] 
    })
}

  addToCart =  (item) => () => {
    const cart = [...this.state.cartItems, item];
    localStorage.setItem('cartItems6', JSON.stringify(cart));
    this.setState({
      cartItems: [...this.state.cartItems, item]})
    }

  removeItem = (index) => () => {
    this.state.cartItems.splice(index, 1);
    localStorage.setItem('cartItems6', JSON.stringify(this.state.cart));
    this.setState({cartItems: this.state.cartItems})
    }

  render() {
    return (
      <div className="App">
        <header className='App-header' style = {{fontSize: 40, fontWeight: 'bold'}}>
          Stuff to Eat
        </header>
        <section style = {{display: 'flex'}}>
          <div style = {{float: 'left'}}>
            <ul style = {{
              listStyle: 'none', 
              padding: 0,
              display: 'flex',
              flexWrap: 'wrap',
              margin: 0
              }}>
            {this.state.inventory.map((item)=> (
            <div style = {{
              border: '2px solid gray',
              width: '140px',
              fontWeight: 'bold',
              margin: 5,
              display: 'grid',
              gridTemplateRows: '.5fr 2fr 1fr .5fr',
              alignItems: 'center',
              justifyItems: 'center'
              }}>
              <li >{item.Name}</li>
              <img style = {{
                width: 120
                }} 
                src={item.Img}
              />
              <p>Price = ${item.Price}</p>
              <AddToCart style = {{
                color: '#07AAFF', 
                backgroundColor: '#282c34',
                height: 25,
                width: 140,
                fontWeight: 'bold',
                cursor: 'pointer'}} 
                onClick = {this.addToCart(item)}  
              />   
            </div>
            ))}
            </ul>
          </div>  
          <div style= {{
            float: 'right',
            width: '40em',
            height: '50em',
            borderLeft: '2px solid gray',
            }}> 
            <h2 style = {{
              textDecoration: 'underline', 
              fontSize: 34}}
            >Cart</h2>
            <ul style = {{
              listStyle: 'none',
              paddingLeft: 0
            }}>
           {this.state.cartItems.map((item, index)=> (
                <div style={{
                  display: 'flex', 
                  justifyContent: 'center',
                  alignItems: 'center'
                  }}>
                  <li style={{
                  paddingRight: 10, 
                  width: 100,
                  fontWeight: 'bold',
                  paddingTop: 5,
                  fontSize: 22
                  }}>
                  {item.Name} 
                  </li>
                  <button style = {{
                    color: 'black', 
                    backgroundColor: 'lightgray',
                    width: 30,
                    height: 20,
                    border: '2px solid black',
                    fontWeight: 'bold',
                    cursor: 'pointer'
                  }} 
                  onClick = {this.removeItem(index)}>X</button>
                </div>))}
            </ul>
            <h3>You have {this.state.cartItems.length} items in your cart.</h3>
            <button 
            style = {{backgroundColor: 'gold',
            border: '2px solid black',
            fontSize: 16,
            fontWeight: 'bold',
            width: 120,
            cursor: 'pointer'}}
            onClick={this.checkoutAlert}>Complete Purchase</button>
          </div>
        </section>
      </div>
    );
  }
}
export default App;
